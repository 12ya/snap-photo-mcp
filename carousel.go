...
func registerGenerateInstagramCarousel(s *server.MCPServer) {
	tool := mcp.NewTool("generate_instagram_carousel",
		mcp.WithDescription(
			"Generate an Instagram carousel and host it on localhost for preview and download. "+
				"Returns the previewURL to the generated HTML file.",
		),
		// ── Brand identity ──────────────────────────────────────────────────
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// ── Native Localhost HTTP Endpoint ───────────────────────────────────
		previewURL := fmt.Sprintf("http://localhost:8080/%s/carousel_%s.html", sessionId, ts)
		...
		out := map[string]any{
			...
			"totalSlides":  len(slides),
			"instructions": "Open the previewURL in your browser to view and download your fast native PNG slides.",
		}
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  <style>
    .dl-overlay {position: fixed; bottom: 30px; right: 30px; z-index: 9999;}
    .dl-btn {background: #111; color: #fff; border: 1px solid rgba(255,255,255,0.1); padding: 14px 24px; border-radius: 30px; font-weight: 600; font-size: 14px; cursor: pointer; font-family:sans-serif; box-shadow: 0 10px 30px rgba(0,0,0,0.5); transition: transform 0.2s, background 0.2s;}
    .dl-btn:hover {transform: scale(1.05); background: #222;}
  </style>
</head>
<body>

<div class="dl-overlay">
  <button onclick="downloadSlides(event)" class="dl-btn">⬇ Download Slides</button>
</div>
...
(function () {
  ...
  window.downloadSlides = async function(e) {
    const btn = e.target;
    ...
    for (let i = 0; i < slideEls.length; i++) {
        ...
        const arrow = slideEls[i].querySelector('.swipe-arrow');
        ...
        link.download = "slide_" + (i+1).toString().padStart(2, '0') + ".png";
